...
$(function () {
    ...
    if (totalPanels > 1) {
        ...
        if (transicion == 'vertical') {
            ...
            nextPanel = 'left';
            prevPanel = 'right';
            transMarginRight = -35 + (40 * totalPanels) + 'px';
        }
...
//            'border': 'solid 2px white',
        ...
        for (b = 1; b <= totalPanels; b++) {
            ...
            if (transicion == 'horizontal' || transicion == 'fade') {
                ...
                secCSS = {
/*                    'margin-right': (totalWidth - lilWidth - lilBorder) / (totalPanels - 1) - lilWidth - lilBorder + 'px'*/'margin-right': '5px'
                };
...
/*		alert("dar clic al num");*/
		availablePanels.css(inactiveCSS)
        $(this).css('background', color1)
		//alert($(this).attr('id'));
        activeIndex = $(this).index();
		if (transicion == 'horizontal') {
			$('#fondomovil').animate({left:(0 - (activeIndex + 0) * 20 + '%')});
		}
		if (transicion == 'vertical') {
			$('#fondomovil').animate({top:(0 - (activeIndex + 0) * 20 + '%')});
		}
        transition()
    })
...
function unLock(thisbutton) {
    ...
    $('#secc' + (activeIndex + 1)).css('background-color', color1).css('color', '#fff')
    navigate()
